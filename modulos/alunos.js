...
const getStatus = function (matriculaAluno) {
    ...
    alunos.forEach(item => {
        if (item.matricula.indexOf(matricula) > -1) {
            item.curso.forEach(itemCurso => {
                itemCurso.disciplinas.forEach(itemCursoDisciplinas => {
                    listaStatus.push(
                        {
                            nome: itemCursoDisciplinas.nome,
                            status: itemCursoDisciplinas.status
                        }
                    )
                })
            })
...
const getStatusEst = function (estado) {
    let situacao = estado
    let listaStatusEst = []
    let erro = true

    alunos.forEach(item => {
        if (item.status == situacao) {
            listaStatusEst.push(
                {
                    nome: item.nome,
                    foto: item.foto
...
const getDisciplina = (disciplinaMatricula) => {
    let matricula = disciplinaMatricula
    let disciplina = []
    let json = {}
    let erro = true
...
module.exports = {
    getNames, getAlunoCurso, getStatus, getStatusEst, getDisciplina
}
